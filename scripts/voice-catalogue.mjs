...
const origin = new URL(
  process.env.PUBLISHED_MAP_URL || 'https://turnright.vercel.app',
);
if (origin.protocol !== 'https:')
  throw new Error('Use a published HTTPS campus origin');
...
const manifest = JSON.parse(
  await fetchBytes(new URL('/packages/latest.json', origin)),
);
...
if (
  !asset ||
  hash(campusBytes) !== asset.sha256 ||
  campusBytes.length !== asset.bytes
)
  throw new Error('Published campus failed its integrity check');
const campus = JSON.parse(campusBytes);
const destinationNames = [
  ...new Set(campus.places.map((p) => p.name).filter(Boolean)),
].sort();
const roadNames = [
  ...new Set(campus.graph.edges.map((e) => e.name).filter(Boolean)),
]
  .filter(
    (name) =>
      !/^(campus (road|path)|path|road|footpath|unnamed.*)$/i.test(name.trim()),
  )
  .sort();
if (process.argv.includes('--report')) {
  const pack = JSON.parse(
    await fs.readFile(
      path.join(root, 'web/public/voice/en-GB-v1/manifest.json'),
      'utf8',
    ),
  );
  const missing = {
    sourceVersion: campus.version,
    destinations: destinationNames.filter(
      (n) => !pack.destinations[normalize(n)],
    ),
    roads: roadNames.filter((n) => !pack.roads[normalize(n)]),
  };
  console.log(JSON.stringify(missing, null, 2));
  process.exitCode =
    missing.destinations.length || missing.roads.length ? 1 : 0;
} else {
  ...
  const core = {
    ...
    'destination-ahead':
      'Your destination is ahead. Keep following the highlighted route.',
    'arrive-entrance':
      'You have reached the mapped entrance. Your walking route is complete.',
    'arrive-approach':
    ...
    recovered:
      'Your location is available again. Follow the highlighted route.',
    rerouting: 'You are away from the route. Finding a new walking route.',
    rerouted: 'Your walking route has been updated.',
    'route-unavailable':
      'A new walking route could not be found. Check the map before continuing.',
    'off-route':
      'You are away from the highlighted route. Checking your location.',
    ready:
      'Natural voice directions are on. In about thirty metres, turn left. Then turn right.',
  };
  ...
  for (const name of roadNames)
    roads[normalize(name)] = hash(normalize(name)).slice(0, 12);
  for (const [kind, action] of Object.entries(turns)) {
    for (const phase of ['now', ...Object.keys(distances)]) {
      const lead =
        phase === 'now' ? 'Now, ' : `In about ${distances[phase]} metres, `;
      add(`turn:${kind}:${phase}`, `${lead}${action}.`);
      for (const name of roadNames)
        add(
          `turn:${kind}:${phase}:${roads[normalize(name)]}`,
          `${lead}${action} onto ${name}.`,
        );
    }
    ...
    for (const [second, nextAction] of Object.entries(turns)) {
      ...
      add(
        `chain:${kind}:${second}:soon`,
        `Ahead, ${action}, then ${nextAction}.`,
      );
