...
export async function packagePhotos(data, root, asset, { fetcher = fetch } = {}) {
  ...
  const catalogue = await fs.readFile(catalogueFile, 'utf8').then(JSON.parse).catch((e) => { if (e.code === 'ENOENT') return []; throw e; });
  ...
  for (const photo of photos) {
    if (!/^[a-f0-9]{64}$/.test(photo.sha256) || photo.url !== `/packages/photos/${photo.sha256}.webp` || !photo.license || !photo.author || !photo.attribution || !photo.sourceUrl || !photo.licenseUrl || !buildings.has(resolve(photo.buildingId))) throw Error(`Invalid approved photograph ${photo.id}`);
    if (assets.has(photo.url)) {
      if (assets.get(photo.url).bytes !== photo.bytes) throw Error('Conflicting photograph asset size');
      continue;
    }
    let bytes;
    for (const folder of ['data/release-photos', 'data/photos']) {
      try { bytes = await fs.readFile(path.join(root, folder, `${photo.sha256}.webp`)); break; }
    ...
    if (!bytes && process.env.PUBLISHED_MAP_URL) {
      ...
      const response = await fetcher(new URL(photo.url, origin), { signal: AbortSignal.timeout(30000) });
      if (response.ok) bytes = Buffer.from(await response.arrayBuffer());
    }
    if (!bytes || bytes.length !== photo.bytes || bytes.length > 250 * 1024 || createHash('sha256').update(bytes).digest('hex') !== photo.sha256 || bytes.toString('ascii', 0, 4) !== 'RIFF' || bytes.toString('ascii', 8, 12) !== 'WEBP') throw Error(`Photograph missing or failed integrity: ${photo.id}. No release images may be omitted.`);
  ...
  if (bytes > 20 * 1024 * 1024) console.warn('Photographs exceed 20 MiB; all approved photographs remain included.');
