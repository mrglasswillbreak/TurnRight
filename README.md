...
The owner-confirmed [student walking correction](docs/CAMPUS-ACCESS.md) enables 62 ordinary campus roads while preserving restricted areas and original source tags. It is included in the local package and the [public production map](https://turnright.vercel.app/).

Vercel Hobby and Supabase Free are configured. The [public map](https://turnright.vercel.app/) and [owner editor](https://turnright.vercel.app/admin) are live. The editor requires the allowlisted GitHub account. Production was published at the owner's request on 9 September 2026; physical phone and campus checks remain outstanding. The owner confirmed ArcGIS offline redistribution permission on 8 September 2026. See the [production record](docs/PRODUCTION.md) for deployment checks and the [configuration record](docs/CONFIGURATION.md) for account details and credential rotation dates:
