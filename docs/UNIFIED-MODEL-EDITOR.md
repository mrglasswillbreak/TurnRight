...
![Desktop model workspace in Orbit](assets/screenshots/editor-model-orbit-current-2026-09-25.png)

## Choose a surface

Use the searchable building hierarchy or the mapped-wall selector to choose a wing and wall. The workspace opens in **Orbit**. Selecting a part updates its properties without moving the camera. **Edit surface** aligns the same 3D canvas to the selected wall, roof or footprint; **Orbit** restores the previous camera position. **2D precision** keeps the same editing target and commands and is used automatically when WebGL is unavailable. Wall views face outside, including courtyard walls, and label their A → B or B → A direction. Numeric positions retain their original distance from A; vertical distances are measured from the model base. Selecting a surface or detail does not create a model-history entry.

Desktop uses a collapsible structure tree, centre viewport and right properties panel. At widths up to 900 px, and on touch devices in short landscape viewports, the workspace gives the canvas the remaining screen. A labelled mode selector keeps Details, Appearance, Roof, Outline and Review reachable. Switch between **Orbit / Edit surface / 2D precision / Photo**. Changing layouts keeps the draft and selection.
...
![Nested structure and selected-item grouping actions](assets/screenshots/editor-model-tree-actions-2026-09-25.png)

![Landscape controls beside the model](assets/screenshots/editor-model-landscape-2026-09-25.png)
...
| --- | --- |
| ![Wall text placement and appearance](assets/screenshots/editor-model-wall-text-2026-09-25.png) | ![Roof text following the selected wing](assets/screenshots/editor-model-roof-text-2026-09-25.png) |
...
![Release review in the current owner workspace](assets/screenshots/editor-releases-current-2026-09-25.png)
