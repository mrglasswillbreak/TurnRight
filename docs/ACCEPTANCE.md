...
The local baseline after the [student-access correction](CAMPUS-ACCESS.md) is 219 source records, 206 mapped approaches, 0 connected entrances, 8 path components, and 166 excluded directed segments. Of those approaches, 190 connect to the largest component; 13 places have no approach. Full metrics are in `data/coverage-report.json`. The ArcGIS item's June 2023 modification date is not a survey date. Some names, construction labels, land outlines and access restrictions may be stale.
