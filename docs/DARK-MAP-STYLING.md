...
Original-colour and Editor Appearance follow-up, 15 September 2026: 303 unit tests pass, including reference-height roof-preview validation. Six distinct Chromium cases pass across focused runs (public/editor material and zoom checks, three Settings flows and the documentation capture). Three WebKit cases pass for both renderers and touch Settings. The production-service-worker test passes for saved building appearance and unfinished-roof recovery offline. Client/server type checks, lint and production/PWA build pass with the existing warnings; the lazy-renderer budget remains 119.8 KB gzip against 300 KB. README images are current application captures with local API fixtures; physical Android/iPhone acceptance remains separate.
