...
`map-theme.ts` applies one set of paints to initial map loading, theme changes and editor overlays. It remembers themed layer instances, so zoom and selection updates do not replay unchanged paints or symbol layouts; new or replaced layers still receive the current theme. `map-palette.ts` grades source building colours into slate tones while preserving small hue differences. Roofs, walls, windows and trim remain distinguishable. The browser does not write display colours back into appearances or model packages. Inspector swatches continue showing saved colours.

Enhanced materials are shared by original colour and surface role and recoloured in place. Existing surface metadata identifies each role. For old packages, solid meshes touching the ground shade as walls and solid meshes wholly above ground shade as roofs; ambiguous/detail meshes retain colour-based grading. This compatibility rule changes only colour, never geometry or evidence. It adds no rendering passes or model triangles. Geometry buffers, sector integrity checks, zoom detail thresholds, opacity, model picking and recovery continue to use the existing renderer.
...
Before/after renderer timing is recorded as the `renderer-performance` attachment in the enhanced zoom cases. Timings use the same software-WebGL fixture and record actual Three.js draw calls and indices. They are a regression comparison, not a physical-device frame-rate claim.
