...
**Photo & model**, **Appearance**, **Roof** and **Outline** open the unified building workspace in the relevant mode. **Details** uses a measured wall canvas linked to selectable 3D and a collapsible photograph reference. It supports precise placement, duplicate/copy previews, groups, row/column patterns and independent presets. Mobile uses a full-screen canvas, a labelled mode selector and one focused tool sheet. Wall/3D/Photo views become Plan/3D/Photo for roofs and outlines. Tap selects; Move and Resize explicitly edit. Pinching cancels an unfinished edit before navigation. Expand/Collapse/Done keep the sheet manageable, and focused inputs scroll into view.

The selected detail's **⋯** button, right-click menu or Shift+F10 opens Duplicate, Copy, Paste/copy-to, Delete, Lock and Hide. Mobile **More → Selection actions** retains access from a sheet. The keyboard shortcuts also work outside text inputs. **Height & floors** edits building metres or recorded floor count inside Appearance; custom roof elevations follow proportionally by default, with explicit options to retain them or use building height for an overridden wing. **Review** identifies changed details, evidence gaps and wall assignments that need attention and opens the affected controls.
