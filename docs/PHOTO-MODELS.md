...
> The assessment and publication below are historical, from 24 September. The current verified LASU package is recorded in [Production](PRODUCTION.md). Current [model authoring](MODEL-AUTHORING.md) adds curves, mesh/file workflows and migration 012; [campus imports](CAMPUS-IMPORTS.md) add separate campus data and migrations 013–015.

The photographic assessment used **395 buildings and 39 photographs covering 19 buildings** in baseline `lasu-8577d5c85d2c`. The reviewed result was published as `lasu-313d8a168635`, verified on 24 September 2026. The unified editor updates tools, not that published content. Stable IDs, owner colours, custom roofs, footprints and routing permissions are retained. Photo captions and GPS fixes do not change model revisions.
...
Measurements use Chromium/SwiftShader, not physical Android/iPhone hardware. The coverage report records results and unavailable coverage. Deploy compatible readers first, then inspect, accept and publish the reviewed draft through Releases. For a current campus restore, prepare a new preview with the historical package and other campuses’ current packages; see [Deployment](DEPLOYMENT.md).
