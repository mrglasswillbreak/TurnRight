...
const baseline = await serve('work/mobile-model-baseline'),
...
if (process.argv.includes('--final')) {
  const previous = JSON.parse(
    await fs.readFile('../docs/mobile-model-performance.json', 'utf8'),
  );
  if (
    previous.baselineCommit !== '0fc9dca' ||
    previous.campusSha256 !== asset.sha256
  )
    throw Error('Recorded baseline does not match this comparison');
  runs.push(...previous.runs.filter((r) => r.version === 'baseline'));
}
...
try {
    ...
    process.argv.includes('--capture')
      ...
      process.argv.includes('--capture')
        ...
        for (
          ...
          const compact = version === 'final';
          if (compact) {
            await dialog
              .getByRole('button', { name: 'Choose wall', exact: true })
              .click();
            await dialog.locator(`[data-model-wall="${wallId}"]`).click();
            await dialog
              .getByRole('button', { name: 'Choose wall', exact: true })
              .click();
          } else await dialog.getByLabel('Mapped wall').selectOption(wallId);
          ...
          const field = dialog.getByLabel('Detail name', { exact: true });
          await field.waitFor();
  ...
  if (!process.argv.includes('--capture'))
    await fs.writeFile(
      process.argv.includes('--smoke')
        ? 'work/mobile-model-smoke.json'
        : '../docs/mobile-model-performance.json',
      JSON.stringify(
        {
          date: new Date().toISOString(),
          baselineCommit: '0fc9dca',
          ...
          environment:
            'Windows Chromium headless, SwiftShader software GPU; mobile 390x844, 1x/4x CPU, no physical devices. Input/click capture to post-animation-frame task; network excluded. Both fixtures use real EditorWorkspace and IndexedDB recovery. Baseline is the long mobile form; final uses canvas and sheets. Selection and Wall/3D switches measured separately from network. Software GPU, not a physical mobile device.',
          summary,
          runs,
        },
        null,
