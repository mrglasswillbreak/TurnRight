...
function environment(
  ...
  const env: SensorEnvironment = {
    ...
    supported: { orientation: true, motion: true },
  ...
  const send = (type: string, values: object) =>
    events.dispatchEvent(Object.assign(new Event(type), values));
  return {
    env,
    events,
    document,
...
const make = (env: SensorEnvironment) => {
  const service = new MotionService(env);
...
it('requests both channels synchronously, independent of active listeners', async () => {
  const h = environment({ permissions: true }),
    s = make(h.env);
...
it('retains denied permission across starts and reloads while allowing the other channel', async () => {
  const h = environment({ permissions: true });
  h.orientation.mockResolvedValue('denied');
  const s = make(h.env);
  ...
  await reopened.requestFromGesture(true);
...
it('does not restart after a late permission response or after opting out', async () => {
  ...
  h.orientation.mockImplementation(
    () =>
      new Promise((r) => {
        resolve = r;
      }),
  ...
  const s = make(h.env);
  const release = s.acquire();
  const pending = s.requestFromGesture();
  s.turnOff();
  resolve('granted');
  await pending;
  expect(s.getSnapshot().active).toBe(false);
...
it('owns one listener set and releases it only after the last consumer', async () => {
  ...
  const a = s.acquire(),
    b = s.acquire();
  expect(
    h.add.mock.calls.filter(([name]) => name === 'devicemotion'),
  ).toHaveLength(1);
  a();
  ...
  expect(s.getSnapshot().active).toBe(false);
  expect(
    h.remove.mock.calls.filter(([name]) => name === 'devicemotion'),
  ).toHaveLength(1);
});
...
it('handles unsupported/insecure contexts and thrown permission requests', async () => {
  const insecure = environment({ secure: false, permissions: true }),
    a = make(insecure.env);
  await a.requestFromGesture();
  a.acquire();
  expect(insecure.orientation).not.toHaveBeenCalled();
  expect(a.getSnapshot().active).toBe(false);
  const broken = environment({ permissions: true });
  broken.orientation.mockRejectedValue(new Error('Policy blocked'));
  const b = make(broken.env);
  await b.requestFromGesture();
  b.acquire();
  expect(b.getSnapshot().orientation).toBe('unavailable');
});
it('remembers a NotAllowedError as denial and does not repeat its request on the next start', async () => {
  const h = environment({ permissions: true });
  h.orientation.mockRejectedValue(
    new DOMException('Permission revoked', 'NotAllowedError'),
  );
  const s = make(h.env);
  await s.requestFromGesture();
  expect(s.getSnapshot().orientation).toBe('denied');
  await s.requestFromGesture();
  expect(h.orientation).toHaveBeenCalledTimes(1);
});
it('never persists readings and limits sensor-driven notifications to ten per second', async () => {
  ...
  s.setMode('phone');
  expect(JSON.parse(h.write.mock.calls[0][0])).toEqual({
    enabled: true,
    asked: true,
    denied: [],
    mode: 'phone',
