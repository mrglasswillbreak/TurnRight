...
it('migration 010 retains private owner reads and immutable approvals while adding guarded drafts', async () => {
  ...
  try {
    ...
    expect(
      (
        await db.query(
          `update public.building_media set draft_revision=1,draft_metadata='{"caption":"Unfinished"}' where draft_revision=0 returning id`,
        )
      ).rows,
    ).toHaveLength(1);
    expect(
      (
        await db.query(
          `update public.building_media set draft_revision=1 where draft_revision=0 returning id`,
        )
      ).rows,
    ).toHaveLength(0);
    await db.exec(
      `update public.building_media set status='approved',derivative_path='private/optimized',public_metadata='{}',reviewed_at=now();`,
    );
    await expect(
      db.exec(`update public.building_media set draft_metadata='{}'`),
    ).rejects.toThrow('immutable');
    await db.exec(
      `set role authenticated; set request.jwt.claim.sub='11111111-1111-4111-8111-111111111111';`,
    );
    expect(
      (await db.query('select id from public.building_media')).rows,
    ).toHaveLength(1);
    await db.exec(
