...
function building(id: string): Feature<Polygon> {
  return {
    type: 'Feature',
    properties: { id, kind: 'building', name: 'Hall', height: 0 },
    geometry: {
      type: 'Polygon',
      coordinates: [
        [
...
describe('duplicate review', () => {
  it('consolidates every member of an exact group in one undoable batch', () => {
    ...
    const result = applyEdits(
      data,
      exactDuplicateEdits(data, [], findDuplicateCandidates(data)),
    );
    expect(result.errors).toEqual([]);
    expect(result.data.places).toHaveLength(1);
    expect(result.data.places[0].sourceRefs).toEqual(
      expect.arrayContaining(['library-copy', 'library-third']),
    );
    expect(
      resolvePlaceIds(result.data, ['library-copy', 'library-third']),
    ).toEqual(['library']);
  });
  it('consolidates exact equivalent places and resolves saved IDs and routes', () => {
    const data = duplicatePlaces(),
      candidates = findDuplicateCandidates(data);
    expect(candidates).toHaveLength(1);
    expect(candidates[0].exact).toBe(true);
    const edits = exactDuplicateEdits(data, [], candidates),
      result = applyEdits(data, edits);
    expect(result.errors).toEqual([]);
    expect(result.data.places).toHaveLength(1);
    expect(resolvePlaceIds(result.data, ['library-copy', 'library'])).toEqual([
      'library',
    ]);
    expect(
      findRoutes(result.data, 'a', { placeId: 'library-copy' })[0].nodeIds.at(
        -1,
      ),
    ).toBe('c');
  ...
  it('preserves directed access, closures and entrances through a merge and undo', async () => {
    ...
    data.entrances = [
      {
        id: 'door',
        name: 'Door',
        placeId: 'library-copy',
        buildingId: 'library-copy',
    ...
    const workspace = new EditorWorkspace(
      [],
      async (batch) =>
        batch.edits.map((e) => ({ ...e.edit, updated_at: 'saved' })),
      async () => {},
    );
    ...
    expect(buildingPlace(merged.data, merged.data.map.features[0])?.id).toBe(
      'library',
    );
    expect(merged.data.graph.edges.map((e) => [e.id, e.accessible])).toEqual(
      data.graph.edges.map((e) => [e.id, e.accessible]),
    );
    expect(merged.data.closures).toEqual(data.closures);
    expect(visualEdges(merged.data.graph.edges)).toHaveLength(
  ...
  it('normalizes reversed footprints and remaps associations without removing a same-ID path', () => {
    ...
    data.map.features = [
      ...
      {
        type: 'Feature',
        properties: { kind: 'path', id: 'hall-b' },
        geometry: {
          type: 'LineString',
          coordinates: [
            [3.2, 6.46],
    ...
    const result = applyEdits(
      data,
      duplicateDecision(data, [], candidates[0], 'hall-a'),
    );
    expect(result.errors).toEqual([]);
    expect(result.data.places[0].buildingId).toBe('hall-a');
    expect(
      result.data.map.features.filter((f) => f.properties?.kind === 'building'),
    ).toHaveLength(1);
    expect(
      result.data.map.features.filter((f) => f.properties?.kind === 'path'),
    ).toHaveLength(1);
    expect(result.data.buildingIdAliases).toEqual({ 'hall-b': 'hall-a' });
  });
  it('rejects missing and cyclic survivors', () => {
    const data = duplicatePlaces(),
      candidate = findDuplicateCandidates(data)[0];
    const edits = duplicateDecision(data, [], candidate, 'library');
    edits[1].properties.mergedInto = 'missing';
    expect(applyEdits(data, edits).errors.join(' ')).toContain(
      'missing or cyclic',
    );
    ...
    edits[0] = {
      ...edits[0],
      deleted: true,
      properties: { ...edits[0].properties, mergedInto: 'library-copy' },
    };
    expect(applyEdits(data, edits).errors.join(' ')).toContain(
      'missing or cyclic',
    );
  ...
  it('does not automatically reapply an undone merge after refresh', async () => {
    ...
    expect(findDuplicateCandidates(refreshed, workspace.edits)[0].exact).toBe(
      true,
    );
    expect(
      exactDuplicateEdits(
        refreshed,
        workspace.edits,
        findDuplicateCandidates(refreshed, workspace.edits),
      ),
    ).toEqual([]);
  });
  it('retains merges when refreshed source records bring back removed records', () => {
    const data = duplicatePlaces(),
      edits = exactDuplicateEdits(data, [], findDuplicateCandidates(data));
    ...
    const source = [
      record('meta', 'meta', meta),
      ...places.map((p) => record(p.id, 'place', p)),
      ...map.features.map((f, i) => record(String(i), 'feature', f)),
      ...graph.nodes.map((n) => record(n.id, 'node', n)),
      ...graph.edges.map((e) => record(e.id, 'edge', e)),
    ];
