...
import type { MapEdit, Position } from '../src/types';

function network() {
  const data = campusFixture();
  data.graph.edges.forEach((e) => {
    if (['ab', 'ba', 'bc', 'cb'].includes(e.id)) e.sourceId = 'road';
  });
...
function entrance(id: string, path: MapEdit): MapEdit {
  const coordinates =
    path.geometry.type === 'LineString'
      ? (path.geometry.coordinates[0] as Position)
      : ([0, 0] as Position);
  return {
    id,
    kind: 'entrance',
    ...
    properties: {
      ...
      connection: {
        type: 'node',
        nodeId: path.properties.vertexIds![0],
        coordinates,
      },
    },
  };
...
describe('explicit mapping connections', () => {
  it('splits both directions, keeps closures, and reaches the actual entrance', () => {
    ...
    const descendants = result.data.graph.edges.filter((e) =>
      e.parentEdgeIds?.some((id) => ['ab', 'ba'].includes(id)),
    );
    expect(descendants).toHaveLength(4);
    expect(result.data.closures[0].edgeIds.sort()).toEqual(
      descendants.map((e) => e.id).sort(),
    );
    expect(() => findRoutes(result.data, 'a', { placeId: 'library' })).toThrow(
      /No connected/,
    );
  ...
  it('handles multiple splits and cross-draft references independently of edit order', () => {
    const left = approach('left', 3.20015),
      right = approach('right', 3.20035);
    const edits = [
      left,
      right,
      entrance('left-door', left),
      entrance('right-door', right),
    ];
    const a = applyEdits(network(), edits),
      b = applyEdits(network(), [...edits].reverse());
    expect(a.errors).toEqual([]);
    expect(a.data).toEqual(b.data);
    expect(
      findRoutes(a.data, 'a', { placeId: 'library' })[0].destinationEntranceId,
    ).toBe('left-door');
    expect(
      findRoutes(a.data, 'c', { placeId: 'library' })[0].destinationEntranceId,
    ).toBe('right-door');
    expect(
      findRoutes(a.data, { placeId: 'library' }, 'a')[0].originEntranceId,
    ).toBe('left-door');
  });
  it('does not fall back to an approach when explicit entrances are restricted', () => {
    const path = approach('path', 3.20025),
      door = entrance('door', path);
  ...
  it('resolves connections to a new path created later in the input', () => {
    ...
    const b: MapEdit = {
      ...
      properties: {
        name: 'Branch',
        vertexIds: ['branch-start', 'branch-end'],
        connections: [
          {
            vertexId: 'branch-start',
            target: {
              type: 'node',
              nodeId: 'a-path:door',
              coordinates: [3.20025, 6.4602],
            },
          },
        ],
      },
    ...
    expect(result.errors).toEqual([]);
    expect(
      findRoutes(result.data, 'a', 'branch-end')[0].coordinates.at(-1),
    ).toEqual([3.2003, 6.4603]);
  });
  it('preserves a restricted gap through geometry edits', () => {
    ...
    feature.geometry = {
      type: 'LineString',
      coordinates: ['a', 'b', 'c', 'd'].map(
        (id) => data.graph.nodes.find((n) => n.id === id)!.coordinates,
      ),
    };
  ...
  it('keeps shared junctions together and vertex IDs stable when inserting a vertex', () => {
    ...
    data.map.features.push({
      type: 'Feature',
      properties: { id: 'dc', kind: 'path', name: 'Branch' },
      geometry: {
        type: 'LineString',
        coordinates: [
          data.graph.nodes[3].coordinates,
          data.graph.nodes[2].coordinates,
        ],
      },
    });
    const edit = featureEdit(data, 'path', 'road', [])!;
    ...
    expect(changes[0].properties.vertexIds?.slice(0, 3)).toEqual([
      'a',
      expect.any(String),
      'b',
    ]);
    const onlyMove = geometryEdits(
      edit,
      {
        type: 'LineString',
        coordinates: [
    ...
    expect(
      result.data.graph.nodes.find((n) => n.id === 'c')?.coordinates,
    ).toEqual(moved);
  });
  it('keeps disconnected entrances visible, but blocks publication validation', () => {
    const result = applyEdits(network(), [
      {
        id: 'door',
        kind: 'entrance',
        geometry: { type: 'Point', coordinates: [3.201, 6.4602] },
        properties: { name: 'Door', placeId: 'library' },
      },
    ]);
  ...
  it('only snaps within both the screen radius and the ground-distance limit', () => {
    ...
    expect(snapTarget(data, [3.2002, 6.46002], project)?.target?.type).toBe(
      'segment',
    );
  ...
  it('validates typed references and documented floor counts', () => {
    ...
    path.properties.connections = [
      {
        vertexId: 'not-a-vertex',
        target: { type: 'node', nodeId: 'a', coordinates: [3.2, 6.46] },
      },
    ];
    ...
    expect(
      validateEdit({
        ...path,
        properties: { name: 'x', heightMode: 'floors', floors: 51 },
      }).join(),
    ).toContain('floor count');
  });
});
