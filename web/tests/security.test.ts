...
describe("server authorization", () => {
  it("rejects anonymous admin requests before reading private data", async () => {
    ...
    const res = response();
    await admin({ method: "POST", headers: {}, body: { action: "state" } }, res);
    expect(res.code).toBe(401);
    expect(fetcher).not.toHaveBeenCalled();
  });
  it("rejects an authenticated non-admin", async () => {
    const fetcher = vi.fn(async () => new Response(JSON.stringify({ id: "stranger" })));
    vi.stubGlobal("fetch", fetcher);
    const res = response();
    await admin(
      { method: "POST", headers: { authorization: "Bearer fake" }, body: { action: "export" } },
      res,
    );
  ...
  it("does not trust an administrator UUID without a database allowlist entry", async () => {
    ...
    const res = response();
    await admin(
      { method: "POST", headers: { authorization: "Bearer fake" }, body: { action: "state" } },
      res,
    );
    expect(res.code).toBe(403);
  });
