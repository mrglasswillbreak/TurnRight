...
describe('drawing progress', () => {
  const draft = (points: number[][]): UnfinishedDrawing => ({ id: 'new-path', kind: 'path', properties: {}, geometry: { type: 'LineString', coordinates: points } });
  ...
  it('requires three distinct vertices for buildings', () => {
    const building: UnfinishedDrawing = { ...draft([]), kind: 'building', geometry: { type: 'Polygon', coordinates: [[[3, 6], [4, 6]]] } };
    expect(drawingProgress(building).canFinish).toBe(false);
    building.geometry = { type: 'Polygon', coordinates: [[[3, 6], [4, 6], [4, 7]]] };
