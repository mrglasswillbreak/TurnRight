...
beforeEach(() => {
  ...
  call.mockReset();
  call.mockImplementation(async (_a, p) => {
    const payload = p as { command: string; revisionId: string };
    return payload.command === 'finalize'
      ? {
...
it('queues offline saves and resumes the identical operation', async () => {
  ...
  const operation = r.session.pendingUpload!.revisionId;
  expect(call).not.toHaveBeenCalled();
  const recovered = await loadSurveyLocal('owner', r.session.id);
  expect(recovered?.session.pendingUpload?.revisionId).toBe(operation);
  vi.stubGlobal('navigator', { onLine: true });
  await syncSurvey(r, vi.fn());
  expect(r.session.remoteRevision).toBe(operation);
  expect(r.session.pendingUpload).toBeUndefined();
});
it('retries lost finalize responses without generating another recording', async () => {
  ...
  call.mockImplementation(async (_a, p) => {
    const payload = p as { command: string; revisionId: string };
    if (payload.command === 'finalize') {
      if (lose) {
        lose = false;
        throw new Error('Connection lost');
      }
      return { revisionId: payload.revisionId, status: 'complete' };
    }
    return {};
  });
  await expect(syncSurvey(r, vi.fn())).rejects.toThrow('Connection lost');
  const operation = r.session.pendingUpload!.revisionId;
  await syncSurvey(r, vi.fn());
  expect(r.session.remoteRevision).toBe(operation);
  expect(
    call.mock.calls
      .filter(([, p]) => (p as { command: string }).command === 'begin')
      .map(([, p]) => (p as { revisionId: string }).revisionId),
...
it('saves newer local edits as another revision after an old response arrives', async () => {
  ...
  call.mockImplementation(async (_a, p) => {
    ...
    if (payload.command === 'finalize') {
      if (change) {
        r.session.name = 'Newer local name';
        change = false;
      }
      return { revisionId: payload.revisionId, status: 'complete' };
    }
  ...
  await syncSurvey(r, vi.fn());
  const begins = call.mock.calls.filter(
    ([, p]) => (p as { command: string }).command === 'begin',
  );
  expect(begins).toHaveLength(2);
  expect((begins[1][1] as { metadata: { name: string } }).metadata.name).toBe(
    'Newer local name',
  );
});
it('uses bounded sample chunks', async () => {
  ...
  expect(
    call.mock.calls
      .filter(([, p]) => (p as { command: string }).command === 'chunk')
      .map(([, p]) => (p as { samples: unknown[] }).samples.length),
  ).toEqual([250, 250, 1]);
});
it('detects another tab changing local recovery without overwriting it', async () => {
  const r = newSurvey('owner', 'source');
  await saveSurveyLocal(r.session);
  const db = await openDB('turnright-surveys', 2);
  const stored = await db.get('sessions', ['owner', r.session.id]);
  stored.localVersion++;
  stored.name = 'Other tab';
  await db.put('sessions', stored);
  db.close();
  r.session.name = 'My conflicting edits';
  await expect(saveSurveyLocal(r.session)).rejects.toThrow(/another tab/);
  expect((await loadSurveyLocal('owner', r.session.id))?.session.name).toBe(
    'Other tab',
  );
});
