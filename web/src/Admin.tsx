...
function Editor({ data, dark, state, sources, workspace: store, refresh }: {
  ...
  const updateGeometry = (geometry: Geometry) => {
    ...
    if (!current) return;
    const batch = geometryEdits(current, geometry, validation.data, workspace.edits);
    commit(batch, batch.find((e) => e.id === current.id && e.kind === current.kind)!);
  };
  const remove = () => {
    const current = selectedRef.current;
