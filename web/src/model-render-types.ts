...
import { validTextureSettings } from './model-document.js';
...
export function validModelRenderMaterial(value: ModelRenderMaterial): boolean {
  return (
    !!value &&
    typeof value === 'object' &&
    validTextureSettings(value.textureSettings) &&
    typeof value.id === 'string' &&
    value.id.length <= 240 &&
    /^#[a-f\d]{6}$/i.test(value.colour) &&
    [value.opacity, value.roughness, value.metalness].every(
      (n) => Number.isFinite(n) && n >= 0 && n <= 1,
    ) &&
    typeof value.doubleSided === 'boolean' &&
    (value.emissive === undefined || /^#[a-f\d]{6}$/i.test(value.emissive)) &&
    (value.alphaTest === undefined ||
      (Number.isFinite(value.alphaTest) &&
        value.alphaTest >= 0 &&
        value.alphaTest <= 1)) &&
    (value.maps === undefined ||
      (!!value.maps &&
        ...
        Object.entries(value.maps).every(
          ([slot, url]) =>
            [
              'baseMap',
              'normalMap',
            ...
            (/^(data:image\/(png|jpeg|webp);base64,[a-z\d+/]+={0,2})$/i.test(
              url,
            ) ||
              /^\/packages\/model-texture-[a-f\d]{64}\.(png|jpg|webp)$/.test(
                url,
              )),
        )))
  );
