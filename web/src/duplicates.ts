...
function ringKey(ring: number[][]) {
  ...
  return (
    [points, [...points].reverse()]
      .flatMap((order) =>
        order.map((_, i) =>
          [...order.slice(i), ...order.slice(0, i)].join(';'),
        ),
      )
      .sort()[0] || ''
  );
...
export function geometryKey(geometry: Geometry): string {
  if (geometry.type === 'Polygon')
    return JSON.stringify([
      ringKey(geometry.coordinates[0]),
      geometry.coordinates.slice(1).map(ringKey).sort(),
    ]);
  if (geometry.type === 'MultiPolygon')
    return JSON.stringify(
      geometry.coordinates
        .map((coordinates) => geometryKey({ type: 'Polygon', coordinates }))
        .sort(),
    );
...
function footprintsOverlap(a: Feature, b: Feature) {
  const polygons = (f: Feature) =>
    f.geometry.type === 'Polygon'
      ? [f.geometry.coordinates]
      : f.geometry.type === 'MultiPolygon'
        ? f.geometry.coordinates
        : [];
  for (const p of polygons(a))
    for (const q of polygons(b)) {
      const contains = (point: number[], rings: number[][][]) =>
        inside(point, rings[0]) &&
        !rings.slice(1).some((r) => inside(point, r));
      if (p[0].some((v) => contains(v, q)) || q[0].some((v) => contains(v, p)))
        return true;
      for (let i = 1; i < p[0].length; i++)
        for (let j = 1; j < q[0].length; j++)
          if (intersects(p[0][i - 1], p[0][i], q[0][j - 1], q[0][j]))
            return true;
    }
  return false;
}
...
function entranceSignature(data: CampusData, kind: DuplicateKind, id: string) {
  return JSON.stringify(
    (data.entrances || [])
      .filter((e) =>
        kind === 'place' ? e.placeId === id : e.buildingId === id,
      )
      .map((e) =>
        exactAttributes({ ...e }, [
          'id',
          'source',
          kind === 'place' ? 'placeId' : 'buildingId',
        ]),
      )
      .sort(),
  );
}
function associationSignature(
  data: CampusData,
  kind: DuplicateKind,
  id: string,
) {
      ...
      ? data.map.features
          .filter(
            (f) =>
              ...
              (f.properties?.placeId === id ||
                f.properties?.id === id ||
                data.places.find((p) => p.id === id)?.buildingId ===
                  f.properties?.id),
          )
...
export function findDuplicateCandidates(
  ...
  const keep = new Set(
    edits.flatMap((e) =>
      !e.deleted
        ? (e.properties.duplicateKeepSeparate || []).map(
            (other) => `${e.kind}:${[e.id, other].sort().join('|')}`,
          )
        : [],
    ),
  );
  ...
  for (const place of data.places) {
    const matching = new Set([
      ...(names.get(nameKey(place.name)) || []),
      ...placeIndex.query(nearbyBounds(place.coordinates, 35)),
    ]);
    for (const other of matching) {
      const sameName =
        !!nameKey(place.name) && nameKey(place.name) === nameKey(other.name);
      const words = (s: string) =>
        s
          .toLowerCase()
          .split(/\W+/)
          .filter((w) => w.length >= 4 && w !== 'lasu');
      const nearbyMatch =
      ...
      const exact =
        exactAttributes({ ...place }, excluded) ===
          exactAttributes({ ...other }, excluded) &&
        entranceSignature(data, 'place', place.id) ===
          entranceSignature(data, 'place', other.id) &&
        associationSignature(data, 'place', place.id) ===
          associationSignature(data, 'place', other.id);
      add({
        kind: 'place',
        ids: [other.id, place.id],
        names: [other.name, place.name],
        exact,
        reason: exact
  ...
  for (const feature of data.map.features.filter(
    ...
    for (const other of buildings.query(bounds)) {
      const sameGeometry =
        geometryKey(feature.geometry) === geometryKey(other.geometry);
      if (!sameGeometry && !footprintsOverlap(feature, other)) continue;
      const a = String(other.properties?.id || other.id),
        b = String(feature.properties?.id || feature.id);
      const excluded = [
        'id',
        'source',
        'sourceId',
      ...
      const exact =
        sameGeometry &&
        exactAttributes(feature.properties || {}, excluded) ===
          exactAttributes(other.properties || {}, excluded) &&
        entranceSignature(data, 'building', a) ===
          entranceSignature(data, 'building', b) &&
        associationSignature(data, 'building', a) ===
          associationSignature(data, 'building', b);
      add({
        kind: 'building',
        ids: [a, b],
        names: [
          String(other.properties?.name || 'Unnamed building'),
          String(feature.properties?.name || 'Unnamed building'),
        ],
        reason: exact
          ? 'Identical footprint and attributes'
          : 'Overlapping building footprints',
        exact,
        distance: distance(
          points[0] as Position,
          buildingPoints(other)[0] as Position,
        ),
      });
    }
...
function editable(
  ...
  if (saved && !saved.properties.duplicateReviewOnly)
  ...
  if (kind === 'place') {
    ...
    return {
      ...
      properties: {
        ...
        duplicateKeepSeparate: saved?.properties.duplicateKeepSeparate,
      },
    };
  }
  const f = data.map.features.find(
    (f) => f.properties?.kind === kind && f.properties.id === id,
  );
...
export function duplicateDecision(
  ...
  const [a, b] = candidate.ids.map((id) =>
    editable(data, edits, candidate.kind, id),
  );
  if (!survivor)
    return [a, b].map((e, i) => {
      const saved = edits.find(
        (previous) =>
          previous.id === e.id && previous.kind === e.kind && !previous.deleted,
      );
      return {
        ...e,
        properties: {
          ...e.properties,
          duplicateReviewOnly: !saved || !!saved.properties.duplicateReviewOnly,
          duplicateKeepSeparate: [
            ...new Set([
              ...(e.properties.duplicateKeepSeparate || []),
              candidate.ids[1 - i],
            ]),
          ],
        },
  ...
  if (!candidate.ids.includes(survivor))
    throw new Error('Choose one of these records to keep.');
  const keep = survivor === a.id ? a : b,
    remove = survivor === a.id ? b : a;
  const aliases = [
    ...new Set(
      [
        String(remove.properties.name || ''),
        ...String(keep.properties.aliases || '').split(','),
        ...String(remove.properties.aliases || '').split(','),
      ]
        .map((s) => s.trim())
        .filter((s) => s && s !== keep.properties.name),
    ),
  ];
  const refs = (edit: MapEdit) => [
    edit.id,
    String(edit.properties.sourceId || edit.id),
    ...(Array.isArray(edit.properties.sourceRefs)
      ? (edit.properties.sourceRefs as string[])
      : []),
  ];
  return [
    {
      ...
      properties: {
        ...
        sourceRefs: [...new Set([...refs(keep), ...refs(remove)])],
      },
    },
    {
      ...remove,
      deleted: true,
      properties: { ...remove.properties, mergedInto: keep.id },
    },
  ];
}
export function exactDuplicateEdits(
  data: CampusData,
  edits: MapEdit[],
  ...
  const undone = new Set(
    edits
      .filter(
        (e) =>
          e.deleted && e.properties.revertToSource && e.properties.mergedInto,
      )
      .flatMap((e) => [
        `${e.kind}:${e.id}`,
        `${e.kind}:${e.properties.mergedInto}`,
      ]),
  );
  ...
  for (const candidate of candidates
    .filter(
      (c) => c.exact && !c.ids.some((id) => undone.has(`${c.kind}:${id}`)),
    )
    ...
    for (const edit of duplicateDecision(
      data,
      [...current.values()],
      { ...candidate, ids: [plain(keep), plain(remove)] },
      plain(keep),
    )) {
