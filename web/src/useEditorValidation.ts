...
export function retainCampusSources(previous: CampusData, next: CampusData): CampusData {
  ...
  for (const key of ['map', 'graph', 'boundary', 'places', 'closures', 'entrances'] as const) {
    if (JSON.stringify(previous[key]) === JSON.stringify(next[key])) Object.assign(next, { [key]: previous[key] });
  }
  return next;
}
export function useEditorValidation(base: CampusData, edits: MapEdit[]) {
  ...
  const check = useCallback((snapshot: MapEdit[]) => {
    if (!worker.current) return Promise.reject(new Error('Validation is starting. Please try again.'));
  ...
  return useMemo(() => ({ ...state.result, data: state.base === base ? state.result.data : base, pending: state.base !== base || state.edits !== edits, check }), [state, base, edits, check]);
}
