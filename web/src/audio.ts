...
export class OfflineVoice {
  ...
  async load(version: string) {
    ...
    if (!response.ok)
      throw new Error("Voice files unavailable. Download the campus map to use audio offline.");
  ...
  async play(...names: string[]) {
    ...
    for (const name of names) {
      const url = this.clips[name];
      if (!url || generation !== this.generation) return;
      let buffer = this.buffers.get(url);
      if (!buffer) {
        const response = await fetch(url);
        if (!response.ok) throw new Error("A voice clip could not load.");
  ...
  maneuver(kind: ManeuverKind, meters?: number) {
    const prefix =
      meters && meters >= 15
        ? `in-${[20, 30, 50, 100, 200].reduce((best, n) => (Math.abs(n - meters) < Math.abs(best - meters) ? n : best), 20)}`
        : null;
