...
export const distance = (a: Position, b: Position) => {
  ...
  return Math.hypot((b[0] - a[0]) * rad * Math.cos((a[1] + b[1]) * rad / 2), (b[1] - a[1]) * rad) * 6371000;
};
export const bearing = (a: Position, b: Position) => {
  const rad = Math.PI / 180;
  return (Math.atan2(Math.sin((b[0] - a[0]) * rad) * Math.cos(b[1] * rad), Math.cos(a[1] * rad) * Math.sin(b[1] * rad) - Math.sin(a[1] * rad) * Math.cos(b[1] * rad) * Math.cos((b[0] - a[0]) * rad)) * 180 / Math.PI + 360) % 360;
};
...
export function closestProgress(p: Position, line: Position[], previous = 0) {
  let along = 0, best = { distance: Infinity, progress: 0, point: line[0] };
...
export const meters = (n: number) => n >= 1000 ? `${(n / 1000).toFixed(1)} km` : `${Math.round(n / 5) * 5} m`;
