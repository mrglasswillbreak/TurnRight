...
export function readMapView() {
  try { return localStorage.getItem(key) !== '2d'; } catch { return true; }
}
export function useMapViewPreference() {
  ...
  return [threeD, (value: boolean) => {
    setThreeD(value);
    try { localStorage.setItem(key, value ? '3d' : '2d'); return true; } catch { return false; }
  }] as const;
}
