...
export const MOTION_LIMITS = {
  ...
  stillAcceleration: 0.2,
  stillRotation: 3,
  stillMs: 3000,
  movingAcceleration: 0.8,
  movingMs: 1000,
  gravityMs: 750,
} as const;
...
function projectedHeading(
  alpha: number,
  beta: number,
  gamma: number,
  screen: number,
) {
  const a = radians(alpha),
    b = radians(beta),
    g = radians(gamma),
    s = radians(screen);
  const x = -Math.sin(s),
  ...
  return Math.hypot(east, north) < MOTION_LIMITS.horizontalProjection
    ? null
    : wrapDegrees((Math.atan2(east, north) * 180) / Math.PI);
}
export function normalizeHeading(
  e: OrientationReading,
  screen: number,
  now: number,
): { heading: PhoneHeading | null; reason: string } {
  if (
    !finite(e.beta) ||
    !finite(e.gamma) ||
    !finite(screen) ||
    ...
    return {
      heading: null,
      reason: 'Waiting for usable orientation readings.',
    };
  const top = projectedHeading(0, e.beta, e.gamma, screen);
  if (top === null)
    return {
      heading: null,
      reason: 'Hold the phone flatter to show its direction.',
    };
  if (finite(e.webkitCompassHeading)) {
    ...
    if (
      ...
      (!finite(accuracy) ||
        accuracy < 0 ||
        accuracy > MOTION_LIMITS.compassAccuracy)
    )
      return {
        heading: null,
        reason: 'Compass is unreliable. Move away from magnetic interference.',
      };
    ...
    return {
      heading: {
        degrees: wrapDegrees(e.webkitCompassHeading + angleDelta(natural, top)),
        reference: 'magnetic',
  ...
  if (e.absolute !== true || !finite(e.alpha))
    return {
      heading: null,
      reason: 'A north-referenced compass reading is unavailable.',
    };
  ...
  return {
    heading:
      degrees === null
        ? null
        : { degrees, reference: 'absolute', accuracy: null, timestamp: now },
    reason: '',
  };
}
...
export class MotionProcessor {
  ...
  orientation(e: OrientationReading, screen: number, now: number) {
    ...
    if (
      previous &&
      this.heading &&
      previous.reference === this.heading.reference &&
      screen === this.orientationScreen &&
      now - this.lastOrientation < MOTION_LIMITS.staleMs
    ) {
      const weight =
        1 - Math.exp(-(now - this.lastOrientation) / MOTION_LIMITS.smoothingMs);
      this.heading.degrees = wrapDegrees(
        previous.degrees +
          angleDelta(previous.degrees, this.heading.degrees) * weight,
      );
    }
  ...
  motion(e: MotionReading, now: number) {
    ...
    if (!acceleration) {
      ...
      if (total) {
        if (!this.gravity) {
          this.gravity = [...total];
          this.gravitySince = now;
        }
        const weight =
          1 - Math.exp(-Math.min(dt, 200) / MOTION_LIMITS.gravityMs);
        this.gravity = this.gravity.map((v, i) => v + weight * (total[i] - v));
        if (now - this.gravitySince >= MOTION_LIMITS.gravityMs)
          acceleration = total.map((v, i) => v - this.gravity![i]) as [
            number,
            number,
            number,
          ];
    ...
    const rotation =
      rate && finite(rate.alpha) && finite(rate.beta) && finite(rate.gamma)
    ...
    const rms = Math.sqrt(
      this.window.reduce((n, s) => n + s.acceleration ** 2, 0) /
        this.window.length,
    );
    const rotationRms = this.window.every((s) => s.rotation !== null)
      ? Math.sqrt(
          this.window.reduce((n, s) => n + s.rotation! ** 2, 0) /
            this.window.length,
        )
      : null;
    const candidate: MotionActivity =
      rms > MOTION_LIMITS.movingAcceleration
        ? 'moving'
        : rms < MOTION_LIMITS.stillAcceleration &&
            rotationRms !== null &&
            rotationRms < MOTION_LIMITS.stillRotation
          ? 'still'
          : 'uncertain';
    ...
    this.activity =
      candidate === 'still' && elapsed >= MOTION_LIMITS.stillMs
        ? 'still'
        : candidate === 'moving' && elapsed >= MOTION_LIMITS.movingMs
          ? 'moving'
          : 'uncertain';
    return true;
  }
  tick(now: number) {
    if (now - this.lastAcceleration > MOTION_LIMITS.staleMs)
      this.activity = 'uncertain';
    if (!this.heading) return;
    const rotatedWithoutHeading =
      this.lastTurning > this.lastOrientation &&
      now - this.lastOrientation > MOTION_LIMITS.rotationGraceMs;
    const corroboratedStill =
      now - this.lastRotation <= MOTION_LIMITS.staleMs &&
      this.lastTurning <= this.lastOrientation;
    if (
      rotatedWithoutHeading ||
      (now - this.lastOrientation > MOTION_LIMITS.headingStaleMs &&
        !corroboratedStill)
    ) {
      this.heading = null;
      this.reason =
        'Compass readings interrupted. Waiting for a fresh direction.';
    }
  }
}

export function usableGps(
  ...
  return (
    !!fix &&
    fix.coordinates.every(finite) &&
    finite(fix.accuracy) &&
    fix.accuracy > 0 &&
    fix.accuracy <= accuracy &&
    finite(fix.timestamp) &&
    now - fix.timestamp >= -1000 &&
    now - fix.timestamp <= age
  );
}
export function travelHeading(
  ...
  return usableGps(fix, now) &&
    finite(fix.heading) &&
    finite(fix.speed) &&
    fix.speed > 0.7
    ? wrapDegrees(fix.heading)
    : null;
}
export function cameraDirection(
  mode: MapOrientationMode,
  phone: PhoneHeading | null,
  fix: GpsFix | null | undefined,
  now = Date.now(),
) {
  if (mode === 'north') return { bearing: 0, fallback: '' };
  if (mode === 'phone' && phone)
    return { bearing: phone.degrees, fallback: '' };
  ...
  return {
    ...
    fallback:
      mode === 'phone'
        ? bearing === null
          ? 'Phone-up: holding map direction until compass or GPS direction returns.'
          : 'Phone-up: using GPS travel direction while compass is unavailable.'
        : '',
  };
}
